(function () {
  ...
  var STORAGE_PREFIX = 'tomok_submissions';
  ...
  function uploadFile(file) {
    ...
    return supabase.storage.from(BUCKET).upload(path, file, {
      ...
      if (res && res.error) {
        console.error('[토목건축사] 파일 업로드 실패:', res.error);
  ...
  function insertSubmission(row) {
    ...
    return supabase.from('tomok_submissions').insert(row).select().single();
  }
  ...
  function fetchList() {
    ...
    listFetchInflight = supabase.from('tomok_submissions')
      .select('*')
      .eq('is_deleted', false)
      ...
      .then(function (res) {
        ...
        if (res.error) {
          console.warn('[토목건축사] 목록 조회 실패:', res.error.message);
          return [];
        }
        cachedRows = res.data || [];
        return cachedRows;
      ...
      .catch(function (err) {
        listFetchInflight = null;
        console.warn('[토목건축사] 목록 조회 예외:', err);
        return [];
      });
    ...
    container.innerHTML = rows.map(function (r) {
      ...
      if (r.file_url) {
        if (isPreviewable(r.file_name, r.file_type)) {
          actions += '<button type="button" class="btn btn-sm btn-primary" data-act="tm-preview" data-url="' + escapeAttr(r.file_url) + '" data-name="' + escapeAttr(r.file_name || '') + '" data-type="' + escapeAttr(r.file_type || '') + '">미리보기</button>';
        }
      ...
      if (isExternalArchitectUser()) {
        ...
        managerHtml =
          '<div class="hy-manager-row">' +
            ...
            '<button type="button" class="btn btn-sm btn-primary" data-act="tm-save-manager" data-id="' + mid + '">저장</button>' +
  ...
  function saveManager(id, name) {
    ...
    return supabase.from('tomok_submissions')
      .update({ design_manager: name || null })
      .eq('id', id)
      .then(function (res) {
        if (res.error) {
          console.warn('[토목건축사] 담당자 저장 실패:', res.error.message);
          return false;
        }
        return true;
      })
      .catch(function (err) {
        console.warn('[토목건축사] 담당자 저장 예외:', err);
        return false;
      });
  }
  ...
  function bindForm() {
    var form = $('tm-form');
    if (!form || form._tmBound) return;
    form._tmBound = true;

    var fileInput = $('tm-file');
    var fileInfo = $('tm-file-info');
    ...
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      var msgEl = $('tm-submit-msg');
      var submitBtn = $('tm-submit');
      var title = ($('tm-title').value || '').trim();
      var desc = ($('tm-desc').value || '').trim();
      var contractId = ($('tm-contract').value || '').trim() || null;
      ...
      var emp = getCurrentEmployee();
      var uploadedByName = (emp && emp.name) ? emp.name : '토목건축사';
      ...
      uploadFile(file).then(function (uploaded) {
        ...
        console.error('[토목건축사] 업로드 실패:', err);
        msgEl.className = 'hy-submit-msg hy-err';
    ...
    var list = $('tm-list');
    if (list && !list._tmBound) {
      list._tmBound = true;
      list.addEventListener('click', function (e) {
        var prevBtn = e.target.closest && e.target.closest('[data-act="tm-preview"]');
        if (prevBtn) {
          ensurePreview()(prevBtn.getAttribute('data-url'), prevBtn.getAttribute('data-name'), prevBtn.getAttribute('data-type'));
          return;
        }
        var btn = e.target.closest && e.target.closest('[data-act="tm-save-manager"]');
        if (!btn) return;
  ...
  function subscribeRealtime() {
    ...
    supabase.channel('tm-submissions')
      .on('postgres_changes',
          { event: '*', schema: 'public', table: 'tomok_submissions' },
          function () {
            if (document.getElementById('section-design-tomok')
                && document.getElementById('section-design-tomok').classList.contains('active')) {
              refreshAndRender();
            } else {
              cachedRows = [];
            }
  ...
  window.renderTomokSubmissions = renderSection;
