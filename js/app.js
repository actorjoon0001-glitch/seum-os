(function () {
  ...
  //  showroom/salesperson 은 top-level 컬럼, stage 는 data(jsonb) 안.
  ...
  function syncEcontractsFromSupabase() {
    try {
      ...
      supa
        ...
        .then(function (res) {
          if (!res || res.error || !Array.isArray(res.data)) {
            if (res && res.error) console.error('econtracts 조회 오류:', res.error);
            return;
          }
      ...
      console.error('econtracts 조회 예외:', e);
    }
  }

  // 전자계약서 실시간 구독 — Contract-OS 작성/수정 즉시 목록 갱신.
  // (세움os 프로젝트 realtime publication 에 econtracts 가 추가돼 있어야 동작.
  ...
  function subscribeEcontractsRealtime() {
    ...
    try {
      _econtractRealtimeSub = supa
        .channel('econtracts-realtime')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'econtracts' }, function () {
  ...
  var TW_DEFAULT_TEAMS = [
    ...
    { id: 'gwangju-sales',   name: '광주전시장 영업팀',  team: '영업', showroom: 'gwangju'      },
  ...
  // 현재 로그인 사용자가 선택된 팀의 팀원(팀장 포함)인지 판단
  // 팀 공동 업무일지는 팀원 누구나 작성·수정 가능하도록 사용됨
  function twIsTeamMember(team) {
    if (!team) return false;
    if (twIsAdminLike()) return true;
    var cur = window.seumAuth && window.seumAuth.currentEmployee;
    if (!cur) return false;
    ...
    return members.some(function (m) {
      if (myId && m.id === myId) return true;
      if (!myId && myName && m.name === myName) return true;
      return false;
    });
  }
  ...
  function twSyncCalendarEvent(teamId, date) {
    ...
    if (memberLogs.length > 5) descParts.push('... 외 ' + (memberLogs.length - 5) + '명');

    var payload = {
      id: evId,
      title: title,
      team: team.team || '',
      showroom: team.showroom || '',
      event_type: 'worklog',
      eventType: 'worklog',
      start_date: date,
      end_date: date,
  ...
  function renderTeamWorklog() {
    ...
    if (!_twState.initialized) {
      ...
      _twInitEvents();
      // 이미 저장된 업무일지 전부에 대해 캘린더 이벤트 back-fill (1회)
      try {
        var seen = {};
        twGetWorklogs().forEach(function (w) {
          var k = w.teamId + '|' + w.date;
          if (seen[k]) return;
          seen[k] = true;
          twSyncCalendarEvent(w.teamId, w.date);
        });
      } catch (_e) {}
    }
  }
  ...
  function _twRenderHistory() {
    ...
    var logs = twGetWorklogs();
    if (!allScope && _twState.teamId) {
      logs = logs.filter(function (w) { return w.teamId === _twState.teamId; });
    }
    ...
    var rows = Object.keys(groups).map(function (k) { return groups[k]; })
      .filter(function (g) { return !!teamById[g.teamId]; })
      .sort(function (a, b) {
    ...
    if (!rows.length) {
      tbody.innerHTML = '<tr><td colspan="6" class="no-result-msg">보관된 업무일지가 없습니다.</td></tr>';
      return;
    }
  ...
  function _twInitEvents() {
    ...
    // 히스토리 필터 변경
    var histAll = document.getElementById('tw-history-all-teams');
    if (histAll) histAll.addEventListener('change', _twRenderHistory);
  ...
  function _adminEmpShowroomLabel(id) {
    ...
    var known = { headquarters: '본사 전시장', showroom1: '1전시장', showroom3: '3전시장', showroom4: '4전시장', ganghwa: '강화전시장', gwangju: '광주전시장', andong: '안동전시장' };
    return known[id] || id;
  }
  ...
  function init() {
    ...
    // 팀별 업무일지 합본 섹션 (일일보고 인쇄 시 뒤에 붙음)
    function buildTeamWorklogPrintSection(dateStr) {
      if (!dateStr) return '';
      if (typeof twGetWorklogs !== 'function' || typeof twGetTeams !== 'function') return '';
      ...
      var orderedTeams = teams.filter(function (t) { return usedTeamIds.indexOf(t.id) !== -1; });
      ...
      if (!orderedTeams.length) {
        return '<div class="wl-combined-pagebreak"></div>' + headerHtml +
          '<div class="wl-combined-empty">해당 날짜에 작성된 팀별 업무일지가 없습니다.</div>';
      }

      var esc = escapeHtml;
      var sectionsHtml = orderedTeams.map(function (team) {
        ...
        if (g.members.length) {
          membersHtml = '<div class="wl-team-members">' +
            g.members.slice().sort(function (a, b) {
              ...
              var sec = function (label, text) {
                if (!text || !text.trim()) return '';
                return '<section class="wl-member-sec"><h6>' + label + '</h6><p>' + esc(text) + '</p></section>';
              };
              return '<article class="wl-member-entry">' +
                '<h5>' + esc(m.authorName || '팀원') + '</h5>' +
                sec('오늘 업무', m.tasks) +
                sec('이슈 / 공유', m.issues) +
                sec('내일 계획', m.tomorrow) +
                '</article>';
            }).join('') +
          '</div>';
        }

        return '<section class="wl-team-group">' +
          '<header class="wl-team-head">' +
            '<h3>' + esc(team.name) + '</h3>' +
    ...
    function getCeoDailyPrintStyles() {
      return '@page{size:A4 portrait;margin:12mm}' +
        ...
        // 팀별 업무일지 합본
        '.wl-team-group{border:1px solid #444;border-radius:3px;padding:4mm 5mm;margin-bottom:4mm;break-inside:avoid;page-break-inside:avoid}' +
